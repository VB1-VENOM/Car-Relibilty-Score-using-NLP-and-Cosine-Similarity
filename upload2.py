...
def upload3():
    try:
        ...
        menuy = df2.Vehicle_Title.unique()
        choicey = st.selectbox("Vehicle_Title", menuy)
        df4=df2[df2['Vehicle_Title']==choicey]
        ...
        if st.button('Submit'):
            ...
            st.write("The Number of Positive Sentiments year are : %s"%(n))
            st.write("The Number of Negative Sentiments year are : %s"%(p))
            st.write("Therefore, overall sentiment : %s"%("Positive" if (p>=n) else "Negative"))
            st.write("Reliability score : ",((p/t)*rate)*20)
    except ValueError:
        print("Document not uploaded yet!")
